defmodule TattleWeb.TokenGeneratorLive do
  require Logger
  alias Tattle.Api.Token
  alias Tattle.Api
  alias Tattle.Accounts
  use TattleWeb, :live_view
  import TattleWeb.CoreComponents

  import TattleWeb.Components.TokenModal
  ...
  def render(assigns) do
    ...
    <.token_modal show={@token.open?} id="example-modal" phx-click="close-info-modal">
      ...
      <:body>
        ...
        <p class="text-sm text-slate-600"><%= @token.token %></p>
      </:body>
      <:footer>
        <button phx-click="close-info-modal" class="btn btn-primary">Close</button>
      </:footer>
    </.token_modal>
    ...
    <p class= "text-xs text-gray-500 text-center">  <%= if @rows == nil or @rows == [] do "No Tokens to Display. Generate Tokens to Display Here." else "" end %> </p>
      ...
      case Accounts.get_user_by_session_token(user_token) do
        ...
        user ->
          ...
          tokens_by_curr_user =
            ...
            |> Enum.map(&Map.take(&1, [:id, :token_id, :token_name, :access_level, :expiry, :created_by_user, :revoked?, :inserted_at, :updated_at]))
  ...
  def handle_event("gen_token", %{"token" => params}, socket) do
    ...
    with {:ok, added_entry} <- Api.add_new_access_token(attrs),
      ...
      new_entry = Map.take(added_entry, [:id, :token_id, :token_name, :access_level, :expiry, :created_by_user, :revoked?, :inserted_at, :updated_at])
  ...
  def handle_event("delete_token", %{"token" => token}, socket) do
    case Api.delete_access_token_by_token_id(token["token_id"]) do
      {:ok, deleted_token} ->
        ...
        updated_rows = Enum.reject(socket.assigns.rows, fn t -> t.token_id == deleted_token.token_id end)
